...
static std::vector<float> net_cost, temp_net_cost;

static std::vector<std::vector<Loc>> legal_pos;
...
static std::vector<char> bb_updated_before;
...
static std::vector<float *> point_to_point_timing_cost;
static std::vector<float *> temp_point_to_point_timing_cost;

/* [0..cluster_ctx.clb_nlist.nets().size()-1][1..num_pins-1]. What is the value of the delay */
/* for each connection in the circuit */
static std::vector<float *> point_to_point_delay_cost;
static std::vector<float *> temp_point_to_point_delay_cost;
...
static std::vector<t_bb> bb_coords, bb_num_on_edges;
 ...
 */
static std::vector<std::pair<CellInfo*,BelId>> blocks_affected;
...
///* [0...cluster_ctx.clb_nlist.nets().size()-1] */
static std::vector<t_bb> ts_bb_coord_new, ts_bb_edge_new;
static std::vector<NetInfo*> ts_nets_to_update;
...
static void initial_placement_blocks(/*int * */ std::vector<std::vector<Loc>> &free_locations /*, enum e_pad_loc_type pad_loc_type*/);
static void initial_placement_location(/*int **/ std::vector<std::vector<Loc>> &free_locations, /*ClusterBlockId*/ CellInfo* blk_id,
...
static int setup_blocks_affected(/*ClusterBlockId*/ CellInfo* b_from, int x_to, int y_to, int z_to);

static int find_affected_blocks(/*ClusterBlockId*/ CellInfo* b_from, int x_to, int y_to, int z_to);
...
static float comp_td_point_to_point_delay(/*ClusterNetId*/ NetInfo* net_id, int ipin);

static void comp_td_point_to_point_delays();

static void update_td_cost();

static bool driven_by_moved_block(const /*ClusterNetId*/ NetInfo* net);
...
static bool find_to(/*t_type_ptr type,*/ float rlim,
        ...
        CellInfo* cell_from);
static void find_to_location(/*t_type_ptr*/ IdString type, float rlim,
		int x_from, int y_from,
		int *px_to, int *py_to, int *pz_to);

static void get_non_updateable_bb(/*ClusterNetId*/ NetInfo* net_id, t_bb *bb_coord_new);
...
static void update_net_bb(/*const ClusterNetId*/ NetInfo* net, /*int iblk, const ClusterBlockId*/ CellInfo *blk, /*const ClusterPinId blk_pin*/
         BelId bel_from);
static void update_td_delta_costs(/*const ClusterNetId*/ NetInfo *net, const /*ClusterPinId*/ PortInfo &pin, float& delta_timing_cost, float& delta_delay_cost,
        CellInfo* blk);

static float get_net_cost(/*ClusterNetId*/ NetInfo* net_id, t_bb *bb_ptr);

static void get_bb_from_scratch(/*ClusterNetId*/ NetInfo* net, t_bb *coords,
		t_bb *num_on_edges);

static double get_net_wirelength_estimate(/*ClusterNetId*/ NetInfo* net_id, t_bb *bbptr);
...
//        print_histogram(create_setup_slack_histogram(*timing_info->setup_analyzer()));
	...
	rlim = (float) std::max(device_ctx.grid.width(), device_ctx.grid.height());
...
static void update_rlim(float *rlim, float success_rat, const DeviceGrid& grid) {
	...
	*rlim = (*rlim) * (1. - 0.44 + success_rat);
	upper_lim = std::max(grid.width() - 1, grid.height() - 1);
...
static float starting_t(float *cost_ptr, float *bb_cost_ptr,
	...
	move_lim = std::min(max_moves, (int) cluster_ctx.clb_nlist.blocks().size());
...
//    auto& cluster_ctx = g_vpr_ctx.clustering();
    ...
    auto loc_from = npnr_ctx->getBelLocation(b_from->bel);
	x_from = loc_from.x;
	y_from = loc_from.y;
	z_from = loc_from.z;
	...
	if (imacro != -1) {
        ...
        NPNR_ASSERT(z_swap_offset == 0);

        // Split existing for loop into two passes
        //   -- first checks validity of entire swap
        //   -- second calls setup_blocks_affected
        for (imember = 0; imember < int(pl_macros[imacro].members.size()) && abort_swap == false; imember++) {
            ...
            auto loc_from = npnr_ctx->getBelLocation(curr_b_from->bel);
			curr_x_from = loc_from.x /*place_ctx.block_locs[curr_b_from].x*/;
			curr_y_from = loc_from.y /*place_ctx.block_locs[curr_b_from].y*/;
			curr_z_from = loc_from.z /*place_ctx.block_locs[curr_b_from].z*/;
			...
			if (   curr_x_to < 1 || curr_x_to >= int(device_ctx.grid.width())
				...
				if (!npnr_ctx->isValidBelForCell(pl_macros[imacro].members[imember].blk_index, bel_to)) {
                    abort_swap = true;
                }
                else {
                    ...
                    if (cell_to) {
                        ...
                        else if (!npnr_ctx->isValidBelForCell(cell_to, pl_macros[imacro].members[imember].blk_index->bel)) {
                            abort_swap = true;
                        }
...
//    auto& place_ctx = g_vpr_ctx.mutable_placement();
    ...
    auto b_from = pick_from_block();
    if (!b_from) {
        return ABORTED; //No movable block found
    }

    auto loc = npnr_ctx->getBelLocation(b_from->bel);

	int x_from = loc.x /*place_ctx.block_locs[b_from].x*/;
	int y_from = loc.y /*place_ctx.block_locs[b_from].y*/;
	int z_from = loc.z /*place_ctx.block_locs[b_from].z*/;
	...
	if (abort_swap == false) {
		...
		if (keep_switch == ACCEPTED) {
			...
			*bb_cost = *bb_cost + bb_delta_c;
            ...
            for (auto net_id : ts_nets_to_update) {
				bb_coords[net_id->udata] = ts_bb_coord_new[net_id->udata];
				if (net_id->users.size() >= SMALL_NET)
					bb_num_on_edges[net_id->udata] = ts_bb_edge_new[net_id->udata];

				net_cost[net_id->udata] = temp_net_cost[net_id->udata];

				/* negative temp_net_cost value is acting as a flag. */
				temp_net_cost[net_id->udata] =	-1;
				bb_updated_before[net_id->udata] = NOT_UPDATED_YET;
			}


            // No need to update anything, as we've already done the swap
            //   in setup_blocks_affected
            ...
            // Since we already swapped in setup_blocks_affected(), unswap here;
            //   to prevent npnr assertions firing, unplace all bels first
            for (const auto &b : blocks_affected) {
                auto b_from = b.first;
                npnr_ctx->unbindBel(b_from->bel);
            }
            for (const auto &b : blocks_affected) {
                auto b_from = b.first;
                auto bel = b.second;
...
//    auto& place_ctx = g_vpr_ctx.mutable_placement();
    ...
    std::unordered_set<CellInfo*> tried_from_blocks;
    ...
    while (tried_from_blocks.size() < cluster_ctx.clb_nlist.blocks().size()) {
        ...
        // Since npnr->cells is an unordered_map with ForwardIterators,
        //   it is inefficient to sample randomly from that container.
        //   Instead, I've created a (global) randomly-accessible container 
        //   called npnr_cells that contains the same contents
        auto b_from = npnr_cells.at(vtr::irand(npnr_cells.size() - 1));

        //Record it as tried
        tried_from_blocks.insert(b_from);

        if (b_from->belStrength > STRENGTH_WEAK) {
...
static int find_affected_nets_and_update_costs(e_place_algorithm place_algorithm, float& bb_delta_c, float& timing_delta_c, float& delay_delta_c) {
    ...
    for (const auto &b : blocks_affected) {
        ...
        for (const auto &p : cluster_ctx.clb_nlist.block_pins(blk)) {
            const auto& blk_pin = p.second;
            auto net_id = blk_pin.net;
            if (!net_id) continue;
     ...
     */
    for (auto net_id : ts_nets_to_update) {
        temp_net_cost[net_id->udata] = get_net_cost(net_id, &ts_bb_coord_new[net_id->udata]);
        bb_delta_c += temp_net_cost[net_id->udata] - net_cost[net_id->udata];
    }
...
static void record_affected_net(/*const ClusterNetId*/ NetInfo *net, int& num_affected_nets) {
    //Record effected nets
    if (temp_net_cost[net->udata] < 0.) {
        //Net not marked yet.
        ts_nets_to_update.push_back(net);
        num_affected_nets++;

        //Flag to say we've marked this net.
        temp_net_cost[net->udata] = 1.;
    }
}
...
//    auto& cluster_ctx = g_vpr_ctx.clustering();
    ...
    if (net->users.size() < SMALL_NET) {
        ...
        if(bb_updated_before[net->udata] == NOT_UPDATED_YET) { //Only once per-net
            get_non_updateable_bb(net, &ts_bb_coord_new[net->udata]);
        }
    } else {
...
//        int pin_height_offset = blk_type->pin_height_offset[iblk_pin];
        ...
        auto loc_old = npnr_ctx->getBelLocation(bel_from);
        auto loc_new = npnr_ctx->getBelLocation(blk->bel);

        //Incremental bounding box update
        update_bb(net, &ts_bb_coord_new[net->udata],
                &ts_bb_edge_new[net->udata],
                loc_old.x /* + pin_width_offset*/,
                loc_old.y /* + pin_height_offset*/,
                loc_new.x /* + pin_width_offset*/,
                loc_new.y /* + pin_height_offset*/);
    }
...
static void update_td_delta_costs(/*const ClusterNetId*/ NetInfo *net, const /*ClusterPinId*/ PortInfo &pin, float& delta_timing_cost, float& delta_delay_cost,
    ...
    if (cluster_ctx.clb_nlist.pin_type(pin) == PinType::DRIVER) {
        ...
        for (size_t ipin = 1; ipin < cluster_ctx.clb_nlist.net_pins(net).size(); ipin++) {
            ...
            temp_point_to_point_delay_cost[net->udata][ipin] = temp_delay;

            temp_point_to_point_timing_cost[net->udata][ipin] = get_timing_place_crit(net, ipin) * temp_delay;
            delta_timing_cost += temp_point_to_point_timing_cost[net->udata][ipin] - point_to_point_timing_cost[net->udata][ipin];
            delta_delay_cost += temp_point_to_point_delay_cost[net->udata][ipin] - point_to_point_delay_cost[net->udata][ipin];
        ...
        if (!driven_by_moved_block(net)) {
            int net_pin = cluster_ctx.clb_nlist.pin_net_index(pin, blk);

            float temp_delay = comp_td_point_to_point_delay(net, net_pin);
            temp_point_to_point_delay_cost[net->udata][net_pin] = temp_delay;

            temp_point_to_point_timing_cost[net->udata][net_pin] = get_timing_place_crit(net, net_pin) * temp_delay;
            delta_timing_cost += temp_point_to_point_timing_cost[net->udata][net_pin] - point_to_point_timing_cost[net->udata][net_pin];
            delta_delay_cost += temp_point_to_point_delay_cost[net->udata][net_pin] - point_to_point_delay_cost[net->udata][net_pin];
        }
    }
}
...
//	VTR_ASSERT(type == grid_type);
    auto type = cell_from->type;

	int rlx = std::min<float>(grid.width() - 1, rlim);
	int rly = std::min<float>(grid.height() - 1, rlim); /* Added rly for aspect_ratio != 1 case. */
	active_area = 4 * rlx * rly;

	min_x = std::max<float>(0, x_from - rlx);
	max_x = std::min<float>(grid.width() - 1, x_from + rlx);
	min_y = std::max<float>(0, y_from - rly);
	max_y = std::min<float>(grid.height() - 1, y_from + rly);
...
//	itype = type->index;
	...
	do { /* Until legal */
		...
		if(num_tries >= 2 * std::min<int>(active_area /*/ (type->width * type->height)*/, legal_pos[type.index].size()) + 10) {
        ...
        // HACK HACK HACK
        //   Since nextpnr operates with logic cells (not clusters)
        //   ensure that carry chains are swapped to identical z
		...
		if((x_from == *px_to) && (y_from == *py_to)) {
            ...
            //  *pz_to already set by find_to_location

            if (!npnr_ctx->isValidBelForCell(cell_from, bel_to)) {
                is_legal = false;
            }
            else {
                auto cell_to = npnr_ctx->getBoundBelCell(bel_to);
                if (cell_to) {
                    ...
                    else if (!npnr_ctx->isValidBelForCell(cell_to, cell_from->bel)) {
                        is_legal = false;
                    }
                }
            }
...
static void find_to_location(/*t_type_ptr*/ IdString type, float rlim,
	...
	int max_x = std::min<float>(grid.width() - 1, x_from + rlx);
	int min_y = std::max<float>(0, y_from - rly);
	int max_y = std::min<float>(grid.height() - 1, y_from + rly);

	*pz_to = 0;
	if (int(grid.width() / 4) < rlx || int(grid.height() / 4) < rly || int(legal_pos[itype].size()) < active_area) {
		int ipos = vtr::irand(legal_pos[itype].size() - 1);
        *px_to = legal_pos[itype][ipos].x;
        *py_to = legal_pos[itype][ipos].y;
        *pz_to = legal_pos[itype][ipos].z;
	} else {
		int x_rel = vtr::irand(std::max(0, max_x - min_x));
		int y_rel = vtr::irand(std::max(0, max_y - min_y));
		*px_to = min_x + x_rel;
		*py_to = min_y + y_rel;
...
static void comp_td_point_to_point_delays() {
	...
	for (const auto& net : cluster_ctx.clb_nlist.nets()) {
		...
		for (size_t ipin = 1; ipin < cluster_ctx.clb_nlist.net_pins(net_id).size(); ++ipin) {
			point_to_point_delay_cost[net_id->udata][ipin] = comp_td_point_to_point_delay(net_id, ipin);
		}
	}
}
...
static void update_td_cost() {
    ...
    for (const auto& b : blocks_affected) {
        auto bnum = b.first;
        for (const auto& pin : cluster_ctx.clb_nlist.block_pins(bnum)) {
		    const auto& pin_id = pin.second;
            auto net_id = pin_id.net;
            if (!net_id) continue;
			...
			if (cluster_ctx.clb_nlist.pin_type(pin_id) == PinType::DRIVER) {
				...
				for (size_t ipin = 1; ipin < cluster_ctx.clb_nlist.net_pins(net_id).size(); ipin++) {
					NPNR_ASSERT(temp_point_to_point_delay_cost[net_id->udata][ipin] >= 0);
					point_to_point_delay_cost[net_id->udata][ipin] = temp_point_to_point_delay_cost[net_id->udata][ipin];
					temp_point_to_point_delay_cost[net_id->udata][ipin] = -1;
					NPNR_ASSERT(temp_point_to_point_timing_cost[net_id->udata][ipin] >= 0);
					point_to_point_timing_cost[net_id->udata][ipin] = temp_point_to_point_timing_cost[net_id->udata][ipin];
					temp_point_to_point_timing_cost[net_id->udata][ipin] = -1;
				}
			} else {
                //This pin is a net sink on a moved block
				...
				if (!driven_by_moved_block(net_id)) {
					int net_pin = cluster_ctx.clb_nlist.pin_net_index(pin_id, bnum);

					NPNR_ASSERT(temp_point_to_point_delay_cost[net_id->udata][net_pin] >= 0);
					point_to_point_delay_cost[net_id->udata][net_pin] = temp_point_to_point_delay_cost[net_id->udata][net_pin];
					temp_point_to_point_delay_cost[net_id->udata][net_pin] = -1;
					NPNR_ASSERT(temp_point_to_point_timing_cost[net_id->udata][net_pin] >= 0);
					point_to_point_timing_cost[net_id->udata][net_pin] = temp_point_to_point_timing_cost[net_id->udata][net_pin];
					temp_point_to_point_timing_cost[net_id->udata][net_pin] = -1;
				}
			}
...
static bool driven_by_moved_block(const /*ClusterNetId*/ NetInfo* net) {
    ...
    auto net_driver_block = cluster_ctx.clb_nlist.net_driver_block(net);
    for (const auto& b : blocks_affected) {
        auto blk = b.first;
        if (net_driver_block == blk) {
            return true;
        }
    }
    return false;
...
static void comp_td_costs(float *timing_cost, float *connection_delay_sum) {
	...
	for (const auto& net : cluster_ctx.clb_nlist.nets()) { /* For each net ... */
        auto net_id = net.second.get();
        ...
        for (ipin = 1; ipin < cluster_ctx.clb_nlist.net_pins(net_id).size(); ipin++) {
            ...
            point_to_point_delay_cost[net_id->udata][ipin] = temp_delay_cost;
            temp_point_to_point_delay_cost[net_id->udata][ipin] = -1; /* Undefined */

            point_to_point_timing_cost[net_id->udata][ipin] = temp_timing_cost;
            temp_point_to_point_timing_cost[net_id->udata][ipin] = -1; /* Undefined */
...
static float comp_bb_cost(e_cost_methods method) {
	...
	for (auto& n : cluster_ctx.clb_nlist.nets()) { /* for each net ... */
        ...
        if (!cluster_ctx.clb_nlist.net_is_global(net_id)) { /* Do only if not global. */
			...
			if (net_id->users.size() >= SMALL_NET && method == NORMAL) {
				get_bb_from_scratch(net_id, &bb_coords[net_id->udata],
					&bb_num_on_edges[net_id->udata]);
			}
			else {
				get_non_updateable_bb(net_id, &bb_coords[net_id->udata]);
			}

			net_cost[net_id->udata] = get_net_cost(net_id, &bb_coords[net_id->udata]);
			cost += net_cost[net_id->udata];
			if (method == CHECK)
				expected_wirelength += get_net_wirelength_estimate(net_id, &bb_coords[net_id->udata]);
		}
	}
...
//	free_fast_cost_update();
	...
	if (placer_opts.place_algorithm == PATH_TIMING_DRIVEN_PLACE
		...
		for (const auto& net : cluster_ctx.clb_nlist.nets()) {
			...
			point_to_point_timing_cost[net_id->udata]++;
			free(point_to_point_timing_cost[net_id->udata]);

			temp_point_to_point_timing_cost[net_id->udata]++;
			free(temp_point_to_point_timing_cost[net_id->udata]);

			point_to_point_delay_cost[net_id->udata]++;
			free(point_to_point_delay_cost[net_id->udata]);

			temp_point_to_point_delay_cost[net_id->udata]++;
			free(temp_point_to_point_delay_cost[net_id->udata]);
		}
...
//	auto& device_ctx = g_vpr_ctx.device();
    ...
    ++num_nets; // Because VPR needs it so
...
//
	if (placer_opts.place_algorithm == PATH_TIMING_DRIVEN_PLACE
		...
		for (const auto& net : cluster_ctx.clb_nlist.nets()) {
			 ...
			 * 1 instead of 0 */
			point_to_point_delay_cost[net_id->udata] = (float *)malloc(num_sinks * sizeof(float));
			point_to_point_delay_cost[net_id->udata]--;

			temp_point_to_point_delay_cost[net_id->udata] = (float *)malloc(num_sinks * sizeof(float));
			temp_point_to_point_delay_cost[net_id->udata]--;

			point_to_point_timing_cost[net_id->udata] = (float *)malloc(num_sinks * sizeof(float));
			point_to_point_timing_cost[net_id->udata]--;

			temp_point_to_point_timing_cost[net_id->udata] = (float *)malloc(num_sinks * sizeof(float));
			temp_point_to_point_timing_cost[net_id->udata]--;
		}
		for (const auto& net : cluster_ctx.clb_nlist.nets()) {
            auto net_id = net.second.get();
			for (ipin = 1; ipin < cluster_ctx.clb_nlist.net_pins(net_id).size(); ipin++) {
				point_to_point_delay_cost[net_id->udata][ipin] = 0;
				temp_point_to_point_delay_cost[net_id->udata][ipin] = 0;
			}
		}
	}
...
static void get_bb_from_scratch(/*ClusterNetId*/ NetInfo *net_id, t_bb *coords,
		t_bb *num_on_edges) {
...
//    auto& place_ctx = g_vpr_ctx.placement();
    ...
    auto bnum = net_id->driver.cell;
//	pnum = cluster_ctx.clb_nlist.net_pin_physical_index(net_id, 0);
    auto loc = npnr_ctx->getBelLocation(bnum->bel);
	x = loc.x /*+ cluster_ctx.clb_nlist.block_type(bnum)->pin_width_offset[pnum]*/;
	y = loc.y /*+ cluster_ctx.clb_nlist.block_type(bnum)->pin_height_offset[pnum]*/;

	x = std::max(std::min<int>(x, grid.width() - 2), 1);
	y = std::max(std::min<int>(y, grid.height() - 2), 1);
    ...
    for (auto pin_id : net_id->users) {
		...
		x = std::max(std::min<int>(x, grid.width() - 2), 1); //-2 for no perim channels
		y = std::max(std::min<int>(y, grid.height() - 2), 1); //-2 for no perim channels
...
static float get_net_cost(/*ClusterNetId*/ NetInfo* net_id, t_bb *bbptr) {
...
//    auto& cluster_ctx = g_vpr_ctx.clustering();
	...
	if (net_id->users.size() > 50) {
		crossing = 2.7933 + 0.02616 * (net_id->users.size() - 50);
		/*    crossing = 3.0;    Old value  */
	} else {
		crossing = cross_count[net_id->users.size() - 1];
	}
...
static void update_bb(/*ClusterNetId*/ NetInfo* net_id, t_bb *bb_coord_new,
	...
	if (bb_updated_before[net_id->udata] == GOT_FROM_SCRATCH) {
        /* The net had been updated from scratch, DO NOT update again! */
		return;
	} else if (bb_updated_before[net_id->udata] == NOT_UPDATED_YET) {
        /* The net had NOT been updated before, could use the old values */
		curr_bb_coord = &bb_coords[net_id->udata];
		curr_bb_edge = &bb_num_on_edges[net_id->udata];
		bb_updated_before[net_id->udata] = UPDATED_ONCE;
	} else {
	...
	if (xnew < xold) { /* Move to left. */
		...
		if (xold == curr_bb_coord->xmax) { /* Old position at xmax. */
			if (curr_bb_edge->xmax == 1) {
				...
				bb_updated_before[net_id->udata] = GOT_FROM_SCRATCH;
				return;
			} else {
	...
	if (bb_updated_before[net_id->udata] == NOT_UPDATED_YET) {
		bb_updated_before[net_id->udata] = UPDATED_ONCE;
    }
}
...
//	free(index);
    ...
    for (auto bel : npnr_ctx->getBels()) {
        ...
        legal_pos[itype].push_back(npnr_ctx->getBelLocation(bel));
    }
}
...
//    auto& place_ctx = g_vpr_ctx.mutable_placement();
	...
	if (!npnr_ctx->checkBelAvail(grid[x][y][z])) {
		return (macro_placed);
	}
...
//    auto& device_ctx = g_vpr_ctx.device();
	...
	for (imacro = 0; imacro < int(pl_macros.size()); imacro++) {
		...
		if (free_locations[itype].size() < pl_macros[imacro].members.size()) {
			vpr_throw(VPR_ERROR_PLACE, __FILE__, __LINE__,
					...
					int(pl_macros[imacro].members.size()), cluster_ctx.clb_nlist.block_name(blk_id).c_str(), size_t(blk_id->udata), cluster_ctx.clb_nlist.block_type(blk_id).c_str(npnr_ctx), itype);
		}
		...
		for (itry = 0; itry < macros_max_num_tries && macro_placed == false; itry++) {
			...
			// Choose a random position for the head
			ipos = free_locations[itype][vtr::irand(free_locations[itype].size() - 1)];
		...
		if (macro_placed == false){
            ...
            for (auto ipos : free_locations[itype]) {

				// Try to place the macro
				macro_placed = try_place_macro(itype, ipos, imacro);

                if (macro_placed == true) break;
...
static void initial_placement_blocks(/*int * free_locations, enum e_pad_loc_type pad_loc_type*/
        std::vector<std::vector<Loc>>& free_locations) {
...
//			VTR_ASSERT(place_ctx.grid_blocks[x][y].blocks[z] == EMPTY_BLOCK_ID);
            VTR_ASSERT(npnr_ctx->checkBelAvail(grid[x][y][z]));
...
//			free_locations[itype]--;
            ...
            free_locations.at(itype).pop_back();
...
static void initial_placement_location(/*int **/ std::vector<std::vector<Loc>> &free_locations, /*ClusterBlockId*/ CellInfo *blk_id,
    ...
    auto it = free_locations.at(itype).rbegin();
    auto ie = free_locations.at(itype).rend();
    for (; it != ie; ++it) {
        *px = it->x;
        *py = it->y;
        *pz = it->z;

        if (!npnr_ctx->isValidBelForCell(blk_id, grid[*px][*py][*pz])) 
            continue;

        std::swap(*it, free_locations.at(itype).back());
        return;
    }
    throw;
}
...
//    auto& place_ctx = g_vpr_ctx.mutable_placement();
    ...
    free_locations.resize(legal_pos.size());

    // HACK HACK HACK
    //   Initially, populate free_locations with just z == 0 cells
    //   for carry chain placement
    for (auto it = legal_pos.begin(); it != legal_pos.end(); it++) {
        for (auto jt = it->begin(); jt != it->end(); jt++) {
            if (jt->z == 0)
                free_locations[it - legal_pos.begin()].push_back(*jt);
        }
    }
    ...
    for (auto it = free_locations.begin(); it != free_locations.end(); it++) {
        const auto& src = legal_pos.at(it - free_locations.begin());
	...
	initial_placement_blocks(free_locations);

    // All constraints (including user pads) are placed
    //   before try_place()
