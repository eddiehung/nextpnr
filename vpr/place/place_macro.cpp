...
static /*vtr::vector_map<ClusterBlockId, int>*/ std::vector<int> f_imacro_from_iblk;
...
static void alloc_and_load_imacro_from_iblk(/*t_pl_macro **/ std::vector<t_pl_macro> &macros /*, int num_macros*/);
...
int alloc_and_load_placement_macros(/*t_direct_inf* directs, int num_segments,*/ /*t_pl_macro ***/ std::vector<t_pl_macro> &macros) {
//
...
//	macro = (t_pl_macro *) vtr::malloc (num_macro * sizeof(t_pl_macro));
    ...
    for (auto &b : cluster_ctx.clb_nlist.blocks()) {
        auto head = b.second.get();
        const auto& children = head->constr_children;
        if (children.empty()) continue;
        imacro = macros.size();

        macros.emplace_back();
		macros[imacro].members.resize(children.size()+1);
        macros[imacro].members[0].x_offset = 0;
        macros[imacro].members[0].y_offset = 0;
        macros[imacro].members[0].z_offset = 0;
        macros[imacro].members[0].blk_index = head;

		/* Load the values for each member of the macro */
		for (imember = 1; imember <= int(children.size()); imember++) {
			macros[imacro].members[imember].x_offset = children[imember-1]->constr_x;
			macros[imacro].members[imember].y_offset = children[imember-1]->constr_y;
			macros[imacro].members[imember].z_offset = children[imember-1]->constr_z;
			macros[imacro].members[imember].blk_index = children[imember-1];
		}
	}
...
void get_imacro_from_iblk(int *imacro, ClusterBlockId iblk, std::vector<t_pl_macro> &macros /*, int num_macros*/) {
...
static void alloc_and_load_imacro_from_iblk(/*t_pl_macro **/ std::vector<t_pl_macro> &macros /*, int num_macros*/) {
	...
	f_imacro_from_iblk.resize(cluster_ctx.clb_nlist.blocks().size(), OPEN);

	/* Allocate and initialize the values to OPEN (-1). */
	//for (const auto& b : cluster_ctx.clb_nlist.blocks()) {
    //    auto blk_id = b.second.get();
	//	f_imacro_from_iblk.insert(blk_id, OPEN);
	//}

	/* Load the values */
	for (imacro = 0; imacro < int(macros.size()); imacro++) {
		for (imember = 0; imember < int(macros[imacro].members.size()); imember++) {
			/*ClusterBlockId*/ auto blk_id = macros[imacro].members[imember].blk_index;
			f_imacro_from_iblk[blk_id->udata] = imacro;
		}
	}
}
