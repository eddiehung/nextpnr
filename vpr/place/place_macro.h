...
struct t_pl_macro_member{
	/*ClusterBlockId*/ CellInfo *blk_index;
	int x_offset;
	int y_offset;
	int z_offset;
};
...
struct t_pl_macro {
	//int num_blocks;
	/*t_pl_macro_member**/ std::vector<t_pl_macro_member> members;
};


/* These are the function declarations. */
int alloc_and_load_placement_macros(/*t_direct_inf* directs, int num_segments,*/ /*t_pl_macro ***/ std::vector<t_pl_macro> &chains);
void get_imacro_from_iblk(int *imacro, ClusterBlockId iblk, /*t_pl_macro **/ std::vector<t_pl_macro> &macros /*, int num_macros*/);
